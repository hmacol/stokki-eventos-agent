#!/usr/bin/env bash
# Instalacao do app de confirmacao de rota (Flask) na VPS (Ubuntu 24.04) -- FreshLog
#
# Uso (como root, na VPS):
#   1. Copiar esta pasta para a VPS:  scp -r confirmacao_motoristas/ root@IP_DA_VPS:/opt/confirmacao-motoristas
#   2. ssh root@IP_DA_VPS
#   3. cd /opt/confirmacao-motoristas/infra && bash instalar_vps.sh
#
...
DIR="$(cd "$(dirname "$0")/.." && pwd)"  # raiz do projeto (confirmacao_motoristas/)
cd "$DIR"
...
cp -f infra/Caddyfile /etc/caddy/Caddyfile
...
if [ ! -f infra/.env ]; then
  ...
  echo "de confirmacao_rotas.token_secret / .sync_secret no config.yaml da maquina local."
...
sed -i 's/\r$//' infra/confirmacao-motoristas.service || true
cp -f infra/confirmacao-motoristas.service /etc/systemd/system/confirmacao-motoristas.service
systemctl daemon-reload
systemctl enable confirmacao-motoristas >/dev/null
systemctl restart confirmacao-motoristas

echo
echo "Pronto. Acompanhe com: journalctl -u confirmacao-motoristas -f"
echo "Se o DNS ja aponta para esta VPS, teste: https://confirmacao.freshlogbr.com/r/teste"
