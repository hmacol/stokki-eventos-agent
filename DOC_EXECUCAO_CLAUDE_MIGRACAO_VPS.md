...
**✅ `stokki-romaneios-manha` e `stokki-notificar-transportadoras` validados em produção (17/08, 04:00-04:12).** `romaneios-manha`: 11 PDFs de romaneio reais gerados (rotas do dia, NFs/boletos/canhoteira), 10,2s, 0 erros -- **lembrete: como o print-agent (Fase 7) não existe, alguém precisa baixar e imprimir esses PDFs manualmente pros motoristas** (mesma limitação de sempre, só mudou de onde o PDF é gerado). `notificar-transportadoras`: 1 transportadora notificada por e-mail com a NF real, 0 falhas, sessão Stokki reaproveitada dos cookies salvos (sem precisar logar de novo).

**Pendente de validação:** `stokki-expedicao-frequente` (08:00-19:30, primeiro disparo ainda hoje) e `stokki-sequencia-tarde` (18:00, primeiro ciclo real). Rollback NÃO é simples depois de execuções reais (não-teste) na VPS -- se algo sair errado depois de escrita real, o caminho é corrigir pra frente na VPS, não reverter pro local (o `dados.db` local já ficou pra trás e voltar a usá-lo perderia o que a VPS já processou).
...
**Commitado e deployado (17/08, madrugada):** tudo de hoje (Fase 2 inteira, login/logout, scripts de corte, backup na VPS) commitado em `b1fe48c` + `6faa11d` + `0558cbc` e já no GitHub. Hugo também aproveitou pra adicionar direto no painel: menu "Agendar/reagendar" em Planejamento de Rotas (PUT direto na VUUPT), sidebar de navegação nova, tela de Motoristas (cadastro via VUUPT + `BD_MOTORISTAS.xlsx`) e alarme sonoro de novas exceções na Torre. Testes funcionais de verdade (reagendar pedido, cadastrar motorista) ainda não foram feitos, ficam pro Hugo validar.
...
✅ **Fase 6 — `agente_importacao_stokki`** (e-mail → XML → Stokki) -- **feita 17/08, madrugada, em paralelo pelo Hugo e por mim** (cada um trabalhando direto na VPS ao mesmo tempo -- achado só depois, ao criar um timer duplicado). Deploy em `/opt/agente-importacao-stokki` (sem git remoto, transferido via `git archive` + scp), dois clientes:
- **Empório Quatro Estrelas** (produção já validada localmente antes) -- timer `importacao-stokki-quatro-estrelas.timer` (Seg-Sex, 16:35/17:35/18:35/19:35, criado pelo Hugo), 1º disparo real ainda hoje.
- **Muai** (cliente novo, Hugo testando localmente em `--modo-teste` na mesma noite) -- timer `importacao-stokki-muai.timer` (Seg-Sex, 16:45/17:45/18:45/19:45, 10min depois do Empório pra não colidir na mesma caixa de e-mail).
